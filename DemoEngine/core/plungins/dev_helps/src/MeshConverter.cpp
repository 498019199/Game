...
#endif
#elif defined(ZENGINE_COMPILER_CLANGCL) && (ZENGINE_COMPILER_VERSION >= 160)
...
namespace
{
	...
	quater AiQuatToQuat(aiQuaternion const& v)
	{
		return quater(v.x, v.y, v.z, v.w);
	}

	aiQuaternion QuatToAiQuat(quater const& v)
	{
	...
	float GetInterpTime(std::vector<T> const & vec, float time, size_t& itime_lower, size_t& itime_upper)
	{
		...
		return MathWorker::clamp((diff == 0) ? 0 : (time - vec[itime_lower].first) / diff, 0.0f, 1.0f);
	}

	void MatrixToDQ(float4x4 const & mat, quater& bind_real, quater& bind_dual, float& bind_scale)
	{
		...
		float3 scale;
		float3 trans;
		MathWorker::decompose(scale, bind_real, trans, tmp_mat);

		bind_dual = MathWorker::quat_trans_to_udq(bind_real, trans);
	...
	float4x4 DQToMatrix(quater const& bind_real, quater const& bind_dual, float bind_scale)
	{
		return MathWorker::scaling(bind_scale, bind_scale, bind_scale) * MathWorker::udq_to_matrix(bind_real, bind_dual);
	}
	...
	private:
		...
		void ResampleJointTransform(KeyFrameSet& rkf, float4x4 const& parent_mat, int start_frame, int end_frame, float fps_scale,
			std::vector<std::pair<float, float3>> const & poss, std::vector<std::pair<float, quater>> const & quats,
	...
	void MeshLoader::BuildMeshData(std::vector<aiScene const*> const & scene_lods)
	{
		for (size_t lod = 0; lod < scene_lods.size(); ++ lod)
		{
			for (unsigned int mi = 0; mi < scene_lods[lod]->mNumMeshes; ++ mi)
			{
				...
				{
					normals.resize(mesh->mNumVertices);
					MathWorker::compute_normal(normals.begin(), indices.begin(), indices.end(), positions.begin(), positions.end());
				...
				if ((!has_tangent || !has_binormal) && (first_texcoord != AI_MAX_NUMBER_OF_TEXTURECOORDS))
				{
					tangents.resize(mesh->mNumVertices);
					binormals.resize(mesh->mNumVertices);
					MathWorker::compute_tangent(tangents.begin(), binormals.begin(), indices.begin(), indices.end(),
				...
				if (mesh->mNumBones > 0)
				{
					...
					for (unsigned int bi = 0; bi < mesh->mNumBones; ++ bi)
					{
						...
						if (!found)
						{
							COMMON_ASSERT_MSG(false, "Joint not found!");
						}
					}
	...
	void MeshLoader::BuildJoints(aiScene const * scene)
	{
		...
		{
			float4x4 const mesh_trans = MathWorker::transpose(float4x4(&node->mTransformation.a1)) * parent_mat;
			for (unsigned int i = 0; i < node->mNumMeshes; ++ i)
			{
				...
				for (unsigned int ibone = 0; ibone < mesh->mNumBones; ++ ibone)
				{
					...
					auto const bone_to_mesh = MathWorker::inverse(MathWorker::transpose(float4x4(&bone->mOffsetMatrix.a1))) * mesh_trans;
		...
		std::function<bool(aiNode const *)> mark_joint_nodes = [&mark_joint_nodes, &joint_nodes](aiNode const * node)
		{
			...
			{
				auto joint = MakeSharedPtr<JointComponent>();
				joint->BindParams(quater::Identity(), quater(0, 0, 0, 0), 1);
		...
		for (unsigned int ianim = 0; ianim < scene->mNumAnimations; ++ ianim)
		{
			...
			for (unsigned int ichannel = 0; ichannel < cur_anim->mNumChannels; ++ ichannel)
			{
				...
				if (joint_id > 0)
				{
					...
					std::vector<std::pair<float, quater>> quats;
		...
		std::vector<std::pair<float, float3>> const & scales)
	{
		...
		for (int i = start_frame; i < end_frame; ++ i)
		{
			...
			quater bind_real_resampled(0, 0, 0, 1);
			quater bind_dual_resampled(0, 0, 0, 0);

			if (!scales.empty())
			{
				fraction = GetInterpTime(scales, time, prev_i, i_scale);
				scale_resampled = MathWorker::lerp(scales[prev_i].second, scales[i_scale].second, fraction);
			}
			if (!quats.empty())
			{
				fraction = GetInterpTime(quats, time, prev_i, i_rot);
				bind_real_resampled = MathWorker::slerp(quats[prev_i].second, quats[i_rot].second, fraction);
			}
			if (!poss.empty())
			{
				fraction = GetInterpTime(poss, time, prev_i, i_pos);

				auto bind_dual_prev_i = MathWorker::quat_trans_to_udq(quats[prev_i].second, poss[prev_i].second);
				auto bind_dual_i_pos = MathWorker::quat_trans_to_udq(quats[i_rot].second, poss[i_pos].second);

				auto bind_dq_resampled = MathWorker::sclerp(quats[prev_i].second, bind_dual_prev_i,
			...
			float4x4 const node_mat =
				MathWorker::scaling(scale_resampled) * MathWorker::udq_to_matrix(bind_real_resampled, bind_dual_resampled) * parent_mat;
			MatrixToDQ(node_mat, bind_real_resampled, bind_dual_resampled, scale_resampled.x());

			if (MathWorker::SignBit(bind_real_resampled.w()) < 0)
			{
	...
	void MeshLoader::LoadFromAssimp(std::string_view input_name, MeshMetadata const & metadata)
	{
		...
		for (auto& mesh : meshes_)
		{
			...
			mesh.pos_bb = MathWorker::compute_aabbox(lod0.positions.begin(), lod0.positions.end());
			mesh.tc_bb = MathWorker::compute_aabbox(lod0.texcoords[0].begin(), lod0.texcoords[0].end());
		}
		...
		for (size_t i = nodes_.size(); i > 0; -- i)
		{
			...
			if (node.parent_id >= 0)
			{
				...
				{
					nodes_[node.parent_id].aabb_local |= MathWorker::transform_aabb(node.aabb_local, node.node->TransformToParent());
				}
			}
		}
	}
		...
		for (uint32_t lod = 0; lod < model.NumLods(); ++ lod)
		{
			...
			for (uint32_t i = 0; i < ai_scene.mNumMaterials; ++ i)
			{
...

				{
					...
					float3 const specular = MathWorker::lerp(float3(0.04f, 0.04f, 0.04f),
						float3(mtl.Albedo().x(), mtl.Albedo().y(), mtl.Albedo().z()), mtl.Metalness());

					float const ai_shininess_strength = MathWorker::max3(specular.x(), specular.y(), specular.z());
				...
				if (!mtl.TextureName(RenderMaterial::TS_Height).empty())
				{
					...
					if (!MathWorker::equal<float>(mtl.HeightScale(), 0.06f))
					{
						...
						case EF_ABGR8:
							{
								...
								for (uint32_t j = 0; j < ai_mesh.mNumVertices; ++ j)
								{
									...
									tangent_quat = MathWorker::normalize(tangent_quat);

									auto const tangent = MathWorker::transform_quat(float3(1, 0, 0), tangent_quat);
									auto const binormal = MathWorker::transform_quat(float3(0, 1, 0), tangent_quat)
										* MathWorker::sgn(tangent_quat.w());
									auto const normal = MathWorker::transform_quat(float3(0, 0, 1), tangent_quat);
						...
						case EF_ABGR8:
							{
								...
								for (uint32_t j = 0; j < ai_mesh.mNumVertices; ++ j)
								{
									...
									normal = MathWorker::normalize(normal);
...

				{
					...
					if (!joint_indices.empty())
					{
						...
						for (uint32_t bi = 0; bi < ai_mesh.mNumBones; ++bi)
						{
							...
							float4x4 const offset_mat = MathWorker::transpose(MathWorker::inverse(joint_mat * MathWorker::inverse(node_mat)));
					...
					for (uint32_t ji = 0; ji < skinned_model.NumJoints(); ++ji)
					{
						...
						else if (key_frame_set.frame_id.size() == 1)
						{
							...
							float4x4 const joint_mat =
								MathWorker::scaling(key_frame_set.bind_scale[0], key_frame_set.bind_scale[0], key_frame_set.bind_scale[0]) *
								MathWorker::udq_to_matrix(key_frame_set.bind_real[0], key_frame_set.bind_dual[0]);
					...
					for (size_t ci = 0; ci < non_trivial_joint_indices.size(); ++ci)
					{
						...
						for (uint32_t pi = 0; pi < node_anim.mNumPositionKeys; ++pi)
						{
							...
							float4x4 const key_frame_mat =
								MathWorker::scaling(key_frame_set.bind_scale[pi], key_frame_set.bind_scale[pi], key_frame_set.bind_scale[pi]) *
								MathWorker::udq_to_matrix(key_frame_set.bind_real[pi], key_frame_set.bind_dual[pi]) *
								MathWorker::inverse(parent_mat);

							float3 scale;
							quater rot;
							float3 trans;
							MathWorker::decompose(scale, rot, trans, key_frame_mat);
		...
		XMLNode const& vertices_chunk, uint32_t mesh_index, uint32_t lod, bool recompute_pos_bb, bool recompute_tc_bb)
	{
		...
		for (XMLNode const* vertex_node = vertices_chunk.FirstNode("vertex"); vertex_node; vertex_node = vertex_node->NextSibling("vertex"))
		{
			...
			if (XMLNode const* tangent_quat_node = vertex_node->FirstNode("tangent_quat"))
			{
				...
				mesh_lod.tangents.emplace_back(MathWorker::transform_quat(float3(1, 0, 0), tangent_quat));
				mesh_lod.binormals.emplace_back(MathWorker::transform_quat(float3(0, 1, 0), tangent_quat) * MathWorker::sgn(tangent_quat.w()));
				mesh_lod.normals.emplace_back(MathWorker::transform_quat(float3(0, 0, 1), tangent_quat));
			}
		}
		...
		{
			mesh.pos_bb = MathWorker::compute_aabbox(mesh_lod.positions.begin(), mesh_lod.positions.end());
		}
		if (recompute_tc_bb && (lod == 0))
		{
			mesh.tc_bb = MathWorker::compute_aabbox(mesh_lod.texcoords[0].begin(), mesh_lod.texcoords[0].end());
		}
		if (recompute_tangent_quat)
		{
			...
			for (uint32_t index = 0; index < mesh_lod.positions.size(); ++ index)
			{
				...
				if (!has_tangent)
				{
					COMMON_ASSERT(has_binormal && has_normal);

					tangent = MathWorker::cross(binormal, normal);
				}
				if (!has_binormal)
				{
					COMMON_ASSERT(has_tangent && has_normal);

					binormal = MathWorker::cross(normal, tangent) * mesh_tangents[index].w();
				}
				if (!has_normal)
				{
					COMMON_ASSERT(has_tangent && has_binormal);

					normal = MathWorker::cross(tangent, binormal);
				}
	...
	void MeshLoader::CompileBonesChunk(XMLNode const& bones_chunk)
	{
		for (XMLNode const* bone_node = bones_chunk.FirstNode("bone"); bone_node; bone_node = bone_node->NextSibling("bone"))
		{
			...
			quater joint_bind_real, joint_bind_dual;
			...
			if (XMLNode const* bind_pos_node = bone_node->FirstNode("bind_pos"))
			{
				...
				quater bind_quat(bind_quat_node->Attrib("x")->ValueFloat(), bind_quat_node->Attrib("y")->ValueFloat(),
					bind_quat_node->Attrib("z")->ValueFloat(), bind_quat_node->Attrib("w")->ValueFloat());

				float scale = MathWorker::length(bind_quat);
				bind_quat /= scale;

				joint_bind_dual = MathWorker::quat_trans_to_udq(bind_quat, bind_pos);
			...
			else
			{
				...
				joint_bind_scale = MathWorker::length(joint_bind_real);
				joint_bind_real /= joint_bind_scale;
				if (MathWorker::SignBit(joint_bind_real.w()) < 0)
				{
		...
		for (XMLNode const* kf_node = key_frames_chunk.FirstNode("key_frame"); kf_node; kf_node = kf_node->NextSibling("key_frame"))
		{
			...
			for (XMLNode const* key_node = kf_node->FirstNode("key"); key_node; key_node = key_node->NextSibling("key"))
			{
				...
				if (XMLNode const* pos_node = key_node->FirstNode("pos"))
				{
					...
					bind_real = quater(quat_node->Attrib("x")->ValueFloat(), quat_node->Attrib("y")->ValueFloat(),
						quat_node->Attrib("z")->ValueFloat(), quat_node->Attrib("w")->ValueFloat());

					bind_scale = MathWorker::length(bind_real);
					bind_real /= bind_scale;

					bind_dual = MathWorker::quat_trans_to_udq(bind_real, bind_pos);
				}
				else
				{
	...
	void MeshLoader::LoadFromMeshML(std::string_view input_name, [[maybe_unused]] MeshMetadata const & metadata)
	{
		...
		COMMON_ASSERT(root.Attrib("version") && (root.Attrib("version")->ValueInt() >= 1));
		...
		if (XMLNode const* key_frames_chunk = root.FirstNode("key_frames_chunk"))
		{
			...
			for (size_t i = 0; i < kfs.size(); ++ i)
			{
				...
				if (kf.frame_id.empty())
				{
					...
					quater inv_parent_real;
					quater inv_parent_dual;
					float inv_parent_scale;
					if (joints_[i].parent_id < 0)
					{
						inv_parent_real = quater::Identity();
						inv_parent_dual = quater(0, 0, 0, 0);
						inv_parent_scale = 1;
					}
					else
					{
						auto const& parent_joint = *joints_[joints_[i].parent_id].joint;
						std::tie(inv_parent_real, inv_parent_dual) = MathWorker::inverse(parent_joint.BindReal(), parent_joint.BindDual());
						inv_parent_scale = 1 / parent_joint.BindScale();
					}

					kf.frame_id.push_back(0);
					kf.bind_real.push_back(MathWorker::mul_real(joint.BindReal(), inv_parent_real));
					kf.bind_dual.push_back(MathWorker::mul_dual(joint.BindReal(), joint.BindDual() * inv_parent_scale,
	...
	void MeshLoader::RemoveUnusedJoints()
	{
		...
		for (uint32_t ji = 0; ji < joints_.size(); ++ ji)
		{
			if (joints_used[ji])
			{
				COMMON_ASSERT(joint_mapping[ji] <= ji);
	...
	void MeshLoader::CompressKeyFrameSet(KeyFrameSet& kf)
	{
		...
		COMMON_ASSERT((kf.bind_real.size() == kf.bind_dual.size())
		...
		while (base < static_cast<int>(kf.frame_id.size() - 2))
		{
			...
			quater interpolate_real;
			quater interpolate_dual;
			std::tie(interpolate_real, interpolate_dual) = MathWorker::sclerp(kf.bind_real[base + 0], kf.bind_dual[base + 0],
				kf.bind_real[base + 2], kf.bind_dual[base + 2], factor);
			float const scale = MathWorker::lerp(kf.bind_scale[base + 0], kf.bind_scale[base + 2], factor);

			if (MathWorker::dot(kf.bind_real[base + 1], interpolate_real) < 0)
			{
			...
			quater diff_real;
			quater diff_dual;
			std::tie(diff_real, diff_dual) = MathWorker::inverse(kf.bind_real[base + 1], kf.bind_dual[base + 1]);
			diff_dual = MathWorker::mul_dual(diff_real, diff_dual * scale, interpolate_real, interpolate_dual);
			diff_real = MathWorker::mul_real(diff_real, interpolate_real);
			float diff_scale = scale * kf.bind_scale[base + 1];

			if ((MathWorker::abs(diff_real.x()) < THRESHOLD) && (MathWorker::abs(diff_real.y()) < THRESHOLD)
				&& (MathWorker::abs(diff_real.z()) < THRESHOLD) && (MathWorker::abs(diff_real.w() - 1) < THRESHOLD)
				&& (MathWorker::abs(diff_dual.x()) < THRESHOLD) && (MathWorker::abs(diff_dual.y()) < THRESHOLD)
				&& (MathWorker::abs(diff_dual.z()) < THRESHOLD) && (MathWorker::abs(diff_dual.w()) < THRESHOLD)
				&& (MathWorker::abs(diff_scale - 1) < THRESHOLD))
			{
	...
	RenderModelPtr MeshLoader::Load(MeshMetadata const & metadata)
	{
		...
		{
			global_transform = MathWorker::translation(-nodes_[0].aabb_local.Center())
			...
			for (auto const & mesh : meshes_)
			{
				...
				for (uint32_t lod = 0; lod < num_lods; ++ lod)
				{
					...
					for (auto const & position : mesh_lod.positions)
					{
						...
						{
							static_cast<int16_t>(
								MathWorker::clamp(static_cast<int32_t>(pos.x() * 65535 - 32768), -32768, 32767)),
							static_cast<int16_t>(
								MathWorker::clamp(static_cast<int32_t>(pos.y() * 65535 - 32768), -32768, 32767)),
							static_cast<int16_t>(
								MathWorker::clamp(static_cast<int32_t>(pos.z() * 65535 - 32768), -32768, 32767)),
							32767
						};
					...
					if (normal_stream != -1)
					{
						...
						{
							float3 const normal = MathWorker::normalize(n) * 0.5f + 0.5f;
							uint32_t const compact = MathWorker::clamp(static_cast<uint32_t>(normal.x() * 255 + 0.5f), 0U, 255U)
								| (MathWorker::clamp(static_cast<uint32_t>(normal.y() * 255 + 0.5f), 0U, 255U) << 8)
								| (MathWorker::clamp(static_cast<uint32_t>(normal.z() * 255 + 0.5f), 0U, 255U) << 16);
					...
					if (tangent_quat_stream != -1)
					{
						...
						{
							float3 const tangent = MathWorker::normalize(mesh_lod.tangents[i]);
							float3 const binormal = MathWorker::normalize(mesh_lod.binormals[i]);
							float3 const normal = MathWorker::normalize(mesh_lod.normals[i]);

							quater const tangent_quat = MathWorker::to_quaternion(tangent, binormal, normal, 8);
					...
					if (texcoord_stream != -1)
					{
						for (auto const & tc : mesh_lod.texcoords[0])
						{
							...
							{
								static_cast<int16_t>(MathWorker::clamp(static_cast<int32_t>(tex_coord.x() * 65535 - 32768),
									-32768, 32767)),
								static_cast<int16_t>(MathWorker::clamp(static_cast<int32_t>(tex_coord.y() * 65535 - 32768),
									-32768, 32767)),
							};
					...
					{
						COMMON_ASSERT(blend_indices_stream != -1);
						...
						for (auto const & binding : mesh_lod.joint_bindings)
						{
							...
							for (size_t wi = 0; wi < num; ++ wi)
							{
								...
								weights[wi] = static_cast<uint8_t>(MathWorker::clamp(static_cast<uint32_t>(w * 255 + 0.5f), 0U, 255U));
							}
