...
namespace
{
		...
		int directions, int rings)
	{
		...
		{
			MathWorker::sincos(-i * step, dxdy[i].y(), dxdy[i].x());
		}
		...
		for (int i = 1; i < rings; ++ i)
		{
			for (size_t j = 0; j < ddm.size(); ++ j)
			{
				...
				for (int k = 0; k < directions; ++ k)
				{
					...
					float2 hl0 = MathWorker::lerp(tmp_hm[active][sample_y0 * width + sample_x0], tmp_hm[active][sample_y0 * width + sample_x1], weight_x);
					float2 hl1 = MathWorker::lerp(tmp_hm[active][sample_y1 * width + sample_x0], tmp_hm[active][sample_y1 * width + sample_x1], weight_x);
					float2 h = MathWorker::lerp(hl0, hl1, weight_y);
					float2 ddl0 = MathWorker::lerp(ddm[sample_y0 * width + sample_x0], ddm[sample_y0 * width + sample_x1], weight_x);
					float2 ddl1 = MathWorker::lerp(ddm[sample_y1 * width + sample_x0], ddm[sample_y1 * width + sample_x1], weight_x);
					float2 dd = MathWorker::lerp(ddl0, ddl1, weight_y);
...
void ImagePlane::BumpToNormal(float scale, float amplitude)
{
    ...
    for (uint32_t y = 0; y < height; ++ y)
    {
        for (uint32_t x = 0; x < width; ++ x)
        {
            ...
            float3 normal = MathWorker::normalize(float3(-dx, -dy, scale));
            ...
            if (amplitude > 0)
            {
                ...
                if (delta > 0)
                {
                    float const r = MathWorker::sqrt(1 + delta * delta);
                    occlusion = (r - delta) / r;
                }
            }
...
void ImagePlane::NormalToHeight(float min_z)
{
    ...
    for (uint32_t y = 0; y < height; ++ y)
    {
        for (uint32_t x = 0; x < width; ++ x)
        {
            ...
            {
                normal = MathWorker::normalize(float3(color_32f.r(), color_32f.g(), color_32f.b()) * 2 - 1);
            }
...
void ImagePlane::PrepareNormalCompression(ElementFormat normal_compression_format)
{
    ...
    for (uint32_t y = 0; y < height; ++ y)
    {
        for (uint32_t x = 0; x < width; ++ x)
        {
            ...
            default:
                ZENGINE_UNREACHABLE("Invalid normal compression format.");
            }
...
float ImagePlane::RgbToLum(Color const & clr)
{
    ...
    return MathWorker::dot(float3(clr.r(), clr.g(), clr.b()), RGB_TO_LUM);
}
