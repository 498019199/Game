...
void MeshMetadata::Load(std::string_view name)
{
    ...
    if (metadata_file)
    {
        ...
        if (auto const* pivot_val = root_value.Member("pivot"))
        {
            ...
            COMMON_ASSERT(values.size() >= new_metadata.pivot_.size());
            uint32_t index = 0;
        ...
        if (auto const* translation_val = root_value.Member("translation"))
        {
            ...
            COMMON_ASSERT(values.size() >= new_metadata.translation_.size());
            uint32_t index = 0;
        ...
        if (auto const* rotation_val = root_value.Member("rotation"))
        {
            ...
            COMMON_ASSERT(values.size() >= new_metadata.rotation_.size());
            uint32_t index = 0;
        ...
        if (auto const* scale_val = root_value.Member("scale"))
        {
            ...
            COMMON_ASSERT(values.size() >= new_metadata.scale_.size());
            uint32_t index = 0;
        ...
        if (auto const* axis_mapping_val = root_value.Member("axis_mapping"))
        {
            ...
            COMMON_ASSERT(values.size() >= std::size(new_metadata.axis_mapping_));
            uint32_t index = 0;
...
void MeshMetadata::Save(std::string const & name) const
{
    ...
    if (MathWorker::length_sq(pivot_) > 1e-6f)
    {
    ...
    if (MathWorker::length_sq(translation_) > 1e-6f)
    {
    ...
    if (!MathWorker::equal(rotation_.x(), 0.0f) || !MathWorker::equal(rotation_.y(), 0.0f) || !MathWorker::equal(rotation_.z(), 0.0f) ||
        !MathWorker::equal(rotation_.w(), 1.0f))
    {
    ...
    if (!MathWorker::equal(scale_.x(), 1.0f) || !MathWorker::equal(scale_.y(), 1.0f) || !MathWorker::equal(scale_.z(), 1.0f))
    {
...
{
    transform_ = MathWorker::transformation(&pivot_, static_cast<quater*>(nullptr), &scale_, &pivot_, &rotation_, &translation_);
    ...
    transform_it_ = MathWorker::transpose(MathWorker::inverse(transform_));
}
