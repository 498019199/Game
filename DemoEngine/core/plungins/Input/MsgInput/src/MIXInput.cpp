...
namespace RenderWorker
{
	...
	{
		COMMON_ASSERT(n < motor_speeds_.size());

		motor_speeds_[n] = static_cast<uint16_t>(MathWorker::clamp(static_cast<uint32_t>(motor_speed * 65535), 0U, 65535U));

		XINPUT_VIBRATION vibration{motor_speeds_[0], motor_speeds_[1]};
		XInputSetState(device_id_, &vibration);
	}
} // namespace RenderWorker
#endif // defined(ZENGINE_PLATFORM_WINDOWS)
