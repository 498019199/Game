...
#if defined(ZENGINE_PLATFORM_WINDOWS_DESKTOP) || defined(ZENGINE_PLATFORM_WINDOWS_STORE) || defined(ZENGINE_PLATFORM_ANDROID)
	...
	void MsgInputSensor::OnOrientationDataUpdated(ISensor* sensor, ISensorDataReport* data_report)
	{
			...
			&& (supported != VARIANT_FALSE))
		{
			...
			if (SUCCEEDED(data_report->GetSensorValue(SENSOR_DATA_TYPE_QUATERNION, &prop)))
			{
				...
				{
					COMMON_ASSERT(prop.caub.cElems / sizeof(float) >= 4);
					orientation_quat_ = quater(reinterpret_cast<float const *>(prop.caub.pElems));
				}
