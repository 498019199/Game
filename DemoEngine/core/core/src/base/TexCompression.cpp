...
namespace RenderWorker
{
	...
	uint32_t RGBACluster::PrincipalAxis(float4& axis, float* eig_one, float* eig_two) const
	{
		...
		for (uint32_t i = 0; i < this->NumValidPoints(); ++ i)
		{
			...
			to_pts_max = MathWorker::maximize(to_pts_max, to_pts[i]);
		}
		...
		{
			float4 dir = MathWorker::normalize(upts[1] - upts[0]);
			bool collinear = true;
			for (uint32_t i = 2; i < this->NumValidPoints(); ++ i)
			{
				float4 v = upts[i] - upts[0];
				if (!MathWorker::equal(MathWorker::abs(MathWorker::dot(v, dir)), MathWorker::length(v)))
				{
					collinear = false;
					break;
				}
	...
	void RGBACluster::Recalculate(bool consider_valid)
	{
		...
		for (uint32_t i = 0; i < data_points_.size(); ++ i)
		{
			...
			min_clr_ = MathWorker::minimize(min_clr_, p);
			max_clr_ = MathWorker::maximize(max_clr_, p);
		}
	...
	int RGBACluster::PowerMethod(float4x4 const & mat, float4& eig_vec, float* eig_val) const
	{
		...
		{
			float4 new_b = MathWorker::transform(b, mat);
			...
			// and try again.
			float const new_b_len = MathWorker::length(new_b);
			if (new_b_len < 1e-6f)
			{
				if (bad_eigen_value)
				{
				...
				b = MathWorker::normalize(b);
				bad_eigen_value = true;
				continue;
			}

			new_b = MathWorker::normalize(new_b);

			// If the new eigenvector is close enough to the old one,
			// then we've converged.
			if (MathWorker::equal(1.0f, MathWorker::dot(b, new_b)))
			{
				fixed = true;
			}
		...
		if (eig_val)
		{
			float4 result = MathWorker::transform(b, mat);
			*eig_val = MathWorker::length(result) / MathWorker::length(b);
		}
