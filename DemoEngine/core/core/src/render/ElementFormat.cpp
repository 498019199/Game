 ...
 namespace RenderWorker
 {
     void ConvertToABGR32F(ElementFormat fmt, void const * input, uint32_t num_elems, Color* output)
     {
         ...
         case EF_ARGB8_SRGB:
             for (uint32_t i = 0; i < num_elems; ++ i, p += elem_size, ++ output)
             {
                 *output = Color(MathWorker::srgb_to_linear(p[2] / 255.0f),
                     MathWorker::srgb_to_linear(p[1] / 255.0f),
                     MathWorker::srgb_to_linear(p[0] / 255.0f),
                     MathWorker::srgb_to_linear(p[3] / 255.0f));
             }
             break;
         ...
         case EF_ABGR8_SRGB:
             for (uint32_t i = 0; i < num_elems; ++ i, p += elem_size, ++ output)
             {
                 *output = Color(MathWorker::srgb_to_linear(p[0] / 255.0f),
                     MathWorker::srgb_to_linear(p[1] / 255.0f),
                     MathWorker::srgb_to_linear(p[2] / 255.0f),
                     MathWorker::srgb_to_linear(p[3] / 255.0f));
             }
             break;
 
         default:
             ZENGINE_UNREACHABLE("Not supported element format");
         }
     }
     ...
     void ConvertFromABGR32F(ElementFormat fmt, Color const * input, uint32_t num_elems, void* output)
     {
         ...
         case EF_A8:
             ...
             {
                 *p = static_cast<uint8_t>(MathWorker::clamp(static_cast<int>(input->a() * 255.0f + 0.5f), 0, 255));
             }
             break;
         ...
         case EF_R5G6B5:
             for (uint32_t i = 0; i < num_elems; ++ i, ++ input, p += elem_size)
             {
                 int r = MathWorker::clamp(static_cast<int>(input->r() * 31.0f + 0.5f), 0, 31);
                 int g = MathWorker::clamp(static_cast<int>(input->g() * 63.0f + 0.5f), 0, 63);
                 int b = MathWorker::clamp(static_cast<int>(input->b() * 31.0f + 0.5f), 0, 31);
         ...
         case EF_A1RGB5:
             ...
             {
                 int r = MathWorker::clamp(static_cast<int>(input->r() * 31.0f + 0.5f), 0, 31);
                 int g = MathWorker::clamp(static_cast<int>(input->g() * 31.0f + 0.5f), 0, 31);
                 int b = MathWorker::clamp(static_cast<int>(input->b() * 31.0f + 0.5f), 0, 31);
         ...
         case EF_ARGB4:
             for (uint32_t i = 0; i < num_elems; ++ i, ++ input, p += elem_size)
             {
                 int r = MathWorker::clamp(static_cast<int>(input->r() * 15.0f + 0.5f), 0, 15);
                 int g = MathWorker::clamp(static_cast<int>(input->g() * 15.0f + 0.5f), 0, 15);
                 int b = MathWorker::clamp(static_cast<int>(input->b() * 15.0f + 0.5f), 0, 15);
                 int a = MathWorker::clamp(static_cast<int>(input->a() * 15.0f + 0.5f), 0, 15);
         ...
         case EF_R8:
             for (uint32_t i = 0; i < num_elems; ++ i, ++ input, p += elem_size)
             {
                 *p = static_cast<uint8_t>(MathWorker::clamp(static_cast<int>(input->r() * 255.0f + 0.5f), 0, 255));
             }
             break;
         ...
         case EF_GR8:
             for (uint32_t i = 0; i < num_elems; ++ i, ++ input, p += elem_size)
             {
                 p[0] = static_cast<uint8_t>(MathWorker::clamp(static_cast<int>(input->r() * 255.0f + 0.5f), 0, 255));
                 p[1] = static_cast<uint8_t>(MathWorker::clamp(static_cast<int>(input->g() * 255.0f + 0.5f), 0, 255));
             }
             break;
         ...
         case EF_SIGNED_GR8:
             for (uint32_t i = 0; i < num_elems; ++ i, ++ input, p += elem_size)
             {
                 p[0] = static_cast<int8_t>(MathWorker::clamp(static_cast<int>(input->r() * 127.0f + 0.5f), -127, 127));
                 p[1] = static_cast<int8_t>(MathWorker::clamp(static_cast<int>(input->g() * 127.0f + 0.5f), -127, 127));
             }
             break;
         ...
         case EF_BGR8:
             for (uint32_t i = 0; i < num_elems; ++ i, ++ input, p += elem_size)
             {
                 p[0] = static_cast<uint8_t>(MathWorker::clamp(static_cast<int>(input->r() * 255.0f + 0.5f), 0, 255));
                 p[1] = static_cast<uint8_t>(MathWorker::clamp(static_cast<int>(input->g() * 255.0f + 0.5f), 0, 255));
                 p[2] = static_cast<uint8_t>(MathWorker::clamp(static_cast<int>(input->b() * 255.0f + 0.5f), 0, 255));
             }
             break;
         ...
         case EF_SIGNED_BGR8:
             for (uint32_t i = 0; i < num_elems; ++ i, ++ input, p += elem_size)
             {
                 p[0] = static_cast<int8_t>(MathWorker::clamp(static_cast<int>(input->r() * 127.0f + 0.5f), -127, 127));
                 p[1] = static_cast<int8_t>(MathWorker::clamp(static_cast<int>(input->g() * 127.0f + 0.5f), -127, 127));
                 p[2] = static_cast<int8_t>(MathWorker::clamp(static_cast<int>(input->b() * 127.0f + 0.5f), -127, 127));
             }
             break;
         ...
         case EF_ARGB8:
             for (uint32_t i = 0; i < num_elems; ++ i, ++ input, p += elem_size)
             {
                 p[0] = static_cast<uint8_t>(MathWorker::clamp(static_cast<int>(input->b() * 255.0f + 0.5f), 0, 255));
                 p[1] = static_cast<uint8_t>(MathWorker::clamp(static_cast<int>(input->g() * 255.0f + 0.5f), 0, 255));
                 p[2] = static_cast<uint8_t>(MathWorker::clamp(static_cast<int>(input->r() * 255.0f + 0.5f), 0, 255));
                 p[3] = static_cast<uint8_t>(MathWorker::clamp(static_cast<int>(input->a() * 255.0f + 0.5f), 0, 255));
             }
             break;
         ...
         case EF_SIGNED_ABGR8:
             for (uint32_t i = 0; i < num_elems; ++ i, ++ input, p += elem_size)
             {
                 ...
                 p[1] = static_cast<int8_t>(MathWorker::clamp(static_cast<int>(input->g() * 127.0f + 0.5f), -127, 127));
                 p[2] = static_cast<int8_t>(MathWorker::clamp(static_cast<int>(input->b() * 127.0f + 0.5f), -127, 127));
                 p[3] = static_cast<int8_t>(MathWorker::clamp(static_cast<int>(input->a() * 127.0f + 0.5f), -127, 127));
             }
             break;
         ...
         case EF_A2BGR10:
             for (uint32_t i = 0; i < num_elems; ++ i, ++ input, p += elem_size)
             {
                 int r = MathWorker::clamp(static_cast<int>(input->r() * 1023.0f + 0.5f), 0, 1023);
                 int g = MathWorker::clamp(static_cast<int>(input->g() * 1023.0f + 0.5f), 0, 1023);
                 int b = MathWorker::clamp(static_cast<int>(input->b() * 1023.0f + 0.5f), 0, 1023);
                 int a = MathWorker::clamp(static_cast<int>(input->a() * 3.0f + 0.5f), 0, 3);
         ...
         case EF_SIGNED_A2BGR10:
             for (uint32_t i = 0; i < num_elems; ++ i, ++ input, p += elem_size)
             {
                 uint32_t r = static_cast<uint32_t>(MathWorker::clamp(static_cast<int>(input->r() * 511.0f + 0.5f), -511, 511));
                 uint32_t g = static_cast<uint32_t>(MathWorker::clamp(static_cast<int>(input->g() * 511.0f + 0.5f), -511, 511));
                 uint32_t b = static_cast<uint32_t>(MathWorker::clamp(static_cast<int>(input->b() * 511.0f + 0.5f), -511, 511));
                 uint32_t a = static_cast<uint32_t>(MathWorker::clamp(static_cast<int>(input->a() * 1.0f + 0.5f), -1, 1));
         ...
         case EF_R16:
             for (uint32_t i = 0; i < num_elems; ++ i, ++ input, p += elem_size)
             {
                 ...
                 *s = static_cast<uint16_t>(MathWorker::clamp(static_cast<int>(input->r() * 65535.0f + 0.5f), 0, 65535));
             }
             break;
         ...
         case EF_SIGNED_R16:
             for (uint32_t i = 0; i < num_elems; ++ i, ++ input, p += elem_size)
             {
                 ...
                 *s = static_cast<int16_t>(MathWorker::clamp(static_cast<int>(input->r() * 32767.0f + 0.5f), -32767, 32767));
             }
             break;
         ...
         case EF_GR16:
             ...
             {
                 uint16_t* s = reinterpret_cast<uint16_t*>(p);
                 s[0] = static_cast<uint16_t>(MathWorker::clamp(static_cast<int>(input->r() * 65535.0f + 0.5f), 0, 65535));
                 s[1] = static_cast<uint16_t>(MathWorker::clamp(static_cast<int>(input->g() * 65535.0f + 0.5f), 0, 65535));
             }
             break;
         ...
         case EF_SIGNED_GR16:
             ...
             {
                 int16_t* s = reinterpret_cast<int16_t*>(p);
                 s[0] = static_cast<int16_t>(MathWorker::clamp(static_cast<int>(input->r() * 32767.0f + 0.5f), -32767, 32767));
                 s[1] = static_cast<int16_t>(MathWorker::clamp(static_cast<int>(input->g() * 32767.0f + 0.5f), -32767, 32767));
             }
             break;
         ...
         case EF_BGR16:
             for (uint32_t i = 0; i < num_elems; ++ i, ++ input, p += elem_size)
             {
                 ...
                 s[0] = static_cast<uint16_t>(MathWorker::clamp(static_cast<int>(input->r() * 65535.0f + 0.5f), 0, 65535));
                 s[1] = static_cast<uint16_t>(MathWorker::clamp(static_cast<int>(input->g() * 65535.0f + 0.5f), 0, 65535));
                 s[2] = static_cast<uint16_t>(MathWorker::clamp(static_cast<int>(input->b() * 65535.0f + 0.5f), 0, 65535));
             }
             break;
         ...
         case EF_SIGNED_BGR16:
             for (uint32_t i = 0; i < num_elems; ++ i, ++ input, p += elem_size)
             {
                 ...
                 s[0] = static_cast<int16_t>(MathWorker::clamp(static_cast<int>(input->r() * 32767.0f + 0.5f), -32767, 32767));
                 s[1] = static_cast<int16_t>(MathWorker::clamp(static_cast<int>(input->g() * 32767.0f + 0.5f), -32767, 32767));
                 s[2] = static_cast<int16_t>(MathWorker::clamp(static_cast<int>(input->b() * 32767.0f + 0.5f), -32767, 32767));
             }
             break;
         ...
         case EF_ABGR16:
             for (uint32_t i = 0; i < num_elems; ++ i, ++ input, p += elem_size)
             {
                 ...
                 s[1] = static_cast<uint16_t>(MathWorker::clamp(static_cast<int>(input->g() * 65535.0f + 0.5f), 0, 65535));
                 s[2] = static_cast<uint16_t>(MathWorker::clamp(static_cast<int>(input->b() * 65535.0f + 0.5f), 0, 65535));
                 s[3] = static_cast<uint16_t>(MathWorker::clamp(static_cast<int>(input->a() * 65535.0f + 0.5f), 0, 65535));
             }
             break;
         ...
         case EF_SIGNED_ABGR16:
             for (uint32_t i = 0; i < num_elems; ++ i, ++ input, p += elem_size)
             {
                 ...
                 s[1] = static_cast<int16_t>(MathWorker::clamp(static_cast<int>(input->g() * 32767.0f + 0.5f), -32767, 32767));
                 s[2] = static_cast<int16_t>(MathWorker::clamp(static_cast<int>(input->b() * 32767.0f + 0.5f), -32767, 32767));
                 s[3] = static_cast<int16_t>(MathWorker::clamp(static_cast<int>(input->a() * 32767.0f + 0.5f), -32767, 32767));
             }
             break;
         ...
         case EF_R32:
             ...
             {
                 uint32_t* s = reinterpret_cast<uint32_t*>(p);
                 *s = static_cast<uint32_t>(MathWorker::clamp<uint64_t>(
         ...
         case EF_SIGNED_R32:
             for (uint32_t i = 0; i < num_elems; ++ i, ++ input, p += elem_size)
             {
                 ...
                 *s = static_cast<int32_t>(MathWorker::clamp<int64_t>(
         ...
         case EF_GR32:
             ...
             {
                 uint32_t* s = reinterpret_cast<uint32_t*>(p);
                 s[0] = static_cast<uint32_t>(MathWorker::clamp<uint64_t>(
                     static_cast<uint64_t>(input->r() * static_cast<float>(0xFFFFFFFFULL) + 0.5f), 0ULL, 0xFFFFFFFFULL));
                 s[1] = static_cast<uint32_t>(MathWorker::clamp<uint64_t>(
         ...
         case EF_SIGNED_GR32:
             ...
             {
                 int32_t* s = reinterpret_cast<int32_t*>(p);
                 s[0] = static_cast<int32_t>(MathWorker::clamp<int64_t>(
                     static_cast<int64_t>(input->r() * static_cast<float>(0x7FFFFFFFLL) + 0.5f), -0x7FFFFFFFLL, 0x7FFFFFFFLL));
                 s[1] = static_cast<int32_t>(MathWorker::clamp<int64_t>(
         ...
         case EF_BGR32:
             for (uint32_t i = 0; i < num_elems; ++ i, ++ input, p += elem_size)
             {
                 ...
                 s[1] = static_cast<uint32_t>(MathWorker::clamp<uint64_t>(
                     static_cast<uint64_t>(input->g() * static_cast<float>(0xFFFFFFFFULL) + 0.5f), 0ULL, 0xFFFFFFFFULL));
                 s[2] = static_cast<uint32_t>(MathWorker::clamp<uint64_t>(
         ...
         case EF_SIGNED_BGR32:
             for (uint32_t i = 0; i < num_elems; ++ i, ++ input, p += elem_size)
             {
                 ...
                 s[1] = static_cast<int32_t>(MathWorker::clamp<int64_t>(
                     static_cast<int64_t>(input->g() * static_cast<float>(0x7FFFFFFFLL) + 0.5f), -0x7FFFFFFFLL, 0x7FFFFFFFLL));
                 s[2] = static_cast<int32_t>(MathWorker::clamp<int64_t>(
         ...
         case EF_ABGR32:
             for (uint32_t i = 0; i < num_elems; ++ i, ++ input, p += elem_size)
             {
                 ...
                 s[2] = static_cast<uint32_t>(MathWorker::clamp<uint64_t>(
                     static_cast<uint64_t>(input->b() * static_cast<float>(0xFFFFFFFFULL) + 0.5f), 0ULL, 0xFFFFFFFFULL));
                 s[3] = static_cast<uint32_t>(MathWorker::clamp<uint64_t>(
         ...
         case EF_SIGNED_ABGR32:
             for (uint32_t i = 0; i < num_elems; ++ i, ++ input, p += elem_size)
             {
                 ...
                 s[2] = static_cast<int32_t>(MathWorker::clamp<int64_t>(
                     static_cast<int64_t>(input->b() * static_cast<float>(0x7FFFFFFFLL) + 0.5f), -0x7FFFFFFFLL, 0x7FFFFFFFLL));
                 s[3] = static_cast<int32_t>(MathWorker::clamp<int64_t>(
         ...
         case EF_D24S8:
             ...
             {
                 uint32_t* s = reinterpret_cast<uint32_t*>(p);
                 *s = static_cast<uint32_t>(MathWorker::clamp(static_cast<int>(input->r() * 0x00FFFFFF + 0.5f), 0, 0x00FFFFFF));
             }
             break;
         ...
         case EF_ARGB8_SRGB:
             for (uint32_t i = 0; i < num_elems; ++ i, ++ input, p += elem_size)
             {
                 p[0] = static_cast<uint8_t>(MathWorker::clamp(static_cast<int>(MathWorker::linear_to_srgb(input->b()) * 255.0f + 0.5f), 0, 255));
                 p[1] = static_cast<uint8_t>(MathWorker::clamp(static_cast<int>(MathWorker::linear_to_srgb(input->g()) * 255.0f + 0.5f), 0, 255));
                 p[2] = static_cast<uint8_t>(MathWorker::clamp(static_cast<int>(MathWorker::linear_to_srgb(input->r()) * 255.0f + 0.5f), 0, 255));
                 p[3] = static_cast<uint8_t>(MathWorker::clamp(static_cast<int>(MathWorker::linear_to_srgb(input->a()) * 255.0f + 0.5f), 0, 255));
             }
             break;
         ...
         case EF_ABGR8_SRGB:
             for (uint32_t i = 0; i < num_elems; ++ i, ++ input, p += elem_size)
             {
                 p[0] = static_cast<uint8_t>(MathWorker::clamp(static_cast<int>(MathWorker::linear_to_srgb(input->r()) * 255.0f + 0.5f), 0, 255));
                 p[1] = static_cast<uint8_t>(MathWorker::clamp(static_cast<int>(MathWorker::linear_to_srgb(input->g()) * 255.0f + 0.5f), 0, 255));
                 p[2] = static_cast<uint8_t>(MathWorker::clamp(static_cast<int>(MathWorker::linear_to_srgb(input->b()) * 255.0f + 0.5f), 0, 255));
                 p[3] = static_cast<uint8_t>(MathWorker::clamp(static_cast<int>(MathWorker::linear_to_srgb(input->a()) * 255.0f + 0.5f), 0, 255));
             }
             break;
