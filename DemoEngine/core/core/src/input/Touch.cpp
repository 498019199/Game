...
namespace RenderWorker
{
	...
	{
		COMMON_ASSERT(state >= GS_None && state < GS_NumGestures);
	...
	void InputTouch::GestureNone(float /*elapsed_time*/)
	{
		...
		case 2:
			if (touch_downs_[index_][0] && touch_downs_[index_][1])
			{
				...
				two_finger_start_len_ = MathWorker::length(two_finger_vec_);
	...
	void InputTouch::GestureTap(float elapsed_time)
	{
		...
		case 0:
			{
				...
				float vec_len = MathWorker::length(vec);
		...
		float dist = MathWorker::length(float2(touch_coords_[index_][0] - one_finger_start_pos_));
		...
		case 2:
			{
				...
				float d = MathWorker::abs(MathWorker::dot(two_finger_vec_, vec));
				if (d > ZOOM_THRESHOLD)
				{
					if (MathWorker::abs(1 - MathWorker::abs(vec_len / two_finger_start_len_)) > ZOOM_DISTANCE_THRESHOLD)
					{
	...
	void InputTouch::GestureZoom(float elapsed_time)
	{
		...
		case 2:
			{
				float vec_len = MathWorker::length(float2(touch_coords_[index_][0] - touch_coords_[index_][1]));
				gesture_ = TS_Zoom;
				action_param_->center = float2(touch_coords_[index_][0] + touch_coords_[index_][1]) / 2.0f;
	...
	void InputTouch::GestureRotate(float elapsed_time)
	{
		...
		case 2:
			{
				float2 vec = MathWorker::normalize(float2(touch_coords_[index_][0] - touch_coords_[index_][1]));
				float delta_angle = atan2(two_finger_vec_.y(), two_finger_vec_.x()) - atan2(vec.y(), vec.x());
				if (MathWorker::abs(delta_angle) > 0.01f)
				{
					two_finger_vec_ = vec;
					gesture_ = TS_Rotate;
					action_param_->center = float2(touch_coords_[index_][0] + touch_coords_[index_][1]) / 2.0f;
