...
#define ZENGINE_JOIN(X, Y) ZENGINE_DO_JOIN(X, Y)
#define ZENGINE_DO_STRINGIZE(X) #X
#define ZENGINE_STRINGIZE(X) ZENGINE_DO_STRINGIZE(X)

#if defined(__clang__)
    #define CLANG_VERSION ZENGINE_JOIN(__clang_major__, __clang_minor__)
    ...
    #define GCC_VERSION ZENGINE_JOIN(__GNUC__, __GNUC_MINOR__)
	#if GCC_VERSION >= 110
