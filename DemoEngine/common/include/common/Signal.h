...
namespace CommonWorker
{
	namespace Signal
	{
		...
		class ZENGINE_CORE_API Connection final
		{
			ZENGINE_NONCOPYABLE(Connection);
		...
		public:
            ...
            friend bool operator>(Connection const& lhs, Connection const& rhs) noexcept { return rhs < lhs; } 
            friend bool operator<=(Connection const& lhs, Connection const& rhs) noexcept { return !(rhs < lhs); } 
            friend bool operator>=(Connection const& lhs, Connection const& rhs) noexcept { return !(lhs < rhs); }
		...
		namespace Detail
		{
			...
			class ZENGINE_CORE_API Mutex
			{
				ZENGINE_NONCOPYABLE(Mutex);
			...
			class ZENGINE_CORE_API SignalBase
			{
				ZENGINE_NONCOPYABLE(SignalBase);

				friend class CommonWorker::Signal::Connection;
