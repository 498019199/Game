...
namespace CommonWorker
{
	...
	public:
		...
		std::span<const JsonValue> ValueArray() const
		{
			if (type_ != JsonValueType::Array)
			{
				ZENGINE_UNREACHABLE("Can't be called");
			...
			return std::span<const JsonValue>(array_value);
		}

		std::span<const std::pair<std::string, JsonValue>> ValueObject() const
		{
			if (type_ != JsonValueType::Object)
			{
				ZENGINE_UNREACHABLE("Can't be called");
			...
			return std::span<const std::pair<std::string, JsonValue>>(obj_value.begin(), obj_value.end());
		}
