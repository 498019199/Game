...
class MaterialLoadingDesc : public ResLoadingDesc
{
	struct MaterialData
	{
		...
		float fGlossiness;
		float3 f3Emissive;
		std::array<std::string, Material::TS_TypeCount> TexNames;
	...
	struct RenderMaterialDesc
	{
		...
		std::shared_ptr<MaterialPtr> Mtl;
	};
public:
	MaterialLoadingDesc(const std::string strFileName, XMLNodePtr pFileParse)
	{
		...
		m_MtlDesc.Mtl = MakeSharedPtr<MaterialPtr>();
	}
	...
	void SubThreadStage() override
	{
		...
		if (albedo_node)
		{
			...
			if (attr)
			{
				m_MtlDesc.MtlData->TexNames[Material::TS_Albedo] = std::string(attr->ValueString());
			}
		}
		...
		if (metalness_node)
		{
			...
			if (attr)
			{
				m_MtlDesc.MtlData->TexNames[Material::TS_Metalness] = std::string(attr->ValueString());
			}
		}
		...
		if (glossiness_node)
		{
			...
			if (attr)
			{
				m_MtlDesc.MtlData->TexNames[Material::TS_Glossiness] = std::string(attr->ValueString());
			}
		}
		...
		if (emissive_node)
		{
			...
			if (attr)
			{
				m_MtlDesc.MtlData->TexNames[Material::TS_Emissive] = std::string(attr->ValueString());
			}
		}
		...
		if (normal_node)
		{
			...
			if (attr)
			{
				m_MtlDesc.MtlData->TexNames[Material::TS_Normal] = std::string(attr->ValueString());
			}
		}
		...
		if (height_node)
		{
			...
			if (attr)
			{
				m_MtlDesc.MtlData->TexNames[Material::TS_Height] = std::string(attr->ValueString());
			}
		}
	}
...
{
	return ResLoader::Instance()->SyncQueryT<Material>(MakeSharedPtr<MaterialLoadingDesc>(strFileName, pFileParse));
}
