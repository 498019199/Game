...
#include <cstring>
#include <mutex>
#include "../SDK/LZMA/LzmaLib.h"
#include "../Tool/LZMACodec.hpp"
...
public:
	static LZMALoader& Instance()
	{
		if (!m_LZMAInstance)
		{
			...
			if (!m_LZMAInstance)
			{
				m_LZMAInstance = MakeUniquePtr<LZMALoader>();
			}
		}
		return *m_LZMAInstance;
	}
	...
	{
		return m_LZMACompressFun(dest, destLen, src, srcLen, outProps, outPropsSize, level, dictSize,
			lc, lp, pb, fb, numThreads);
	}
	int LzmaUncompress(unsigned char* dest, size_t* destLen, unsigned char const * src, SizeT* srcLen,
		unsigned char const * props, size_t propsSize)
	{
		return m_LZMAUncompressFun(dest, destLen, src, srcLen, props, propsSize);
	}

	LZMALoader()
	{
#if !(defined(STX_PLATFORM_ANDROID) || defined(STX_PLATFORM_IOS))
		m_LoaderDLL.Load(DLL_PREFIX "LZMA" DLL_SUFFIX);

		m_LZMACompressFun = (LzmaCompressFunc)m_LoaderDLL.GetProcAddress("LzmaCompress");
		m_LZMAUncompressFun = (LzmaUncompressFunc)m_LoaderDLL.GetProcAddress("LzmaUncompress");
#else
		m_LZMACompressFun = ::LzmaCompress;
		m_LZMAUncompressFun = ::LzmaUncompress;
#endif

		BOOST_ASSERT(m_LZMACompressFun);
		BOOST_ASSERT(m_LZMAUncompressFun);
	}

private:
#if !(defined(STX_PLATFORM_ANDROID) || defined(STX_PLATFORM_IOS))
	DllLoader m_LoaderDLL;
#endif
	LzmaCompressFunc m_LZMACompressFun;
	LzmaUncompressFunc m_LZMAUncompressFun;
	static std::unique_ptr<LZMALoader> m_LZMAInstance;
};

std::unique_ptr<LZMALoader> LZMALoader::m_LZMAInstance;
LZMACodec::LZMACodec()
{
}
