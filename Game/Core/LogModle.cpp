...
/*Log管理*****************************************************************/
DEFINE_ENTITY(LogModle);
void LogModle::unlock()
{
    if (m_isthread)
...
LogModle::LogModle()
    :m_log_type(LOG_NONE),m_isthread(false), m_isopen(false)
{
}
...
bool LogModle::OnInit()
{
	return true;
}

bool LogModle::OnShut()
{
	return true;
}

void LogModle::OnExcute(float fTime)
{

}

bool LogModle::open(bool mt /*= true*/, LogLevel nlevel /*= LOG_TRACE*/)
{
    if (Isopen()) 
        return false;
    m_isthread = mt;
...
LogLevel LogModle::SetLogLevel(LogLevel nType)
{
...
int LogModle::AddPrint(Printer *app)
{
    lock();
...
int LogModle::AddFileprint(const char* szfilename, uint64_t len /*= 5 * 1024 * 1024*/, bool ismulfile /*= true*/)
{
...
int LogModle::AddConsoleprint(tostream &os /*= tcout*/)
{
...
bool LogModle::RemovePrint(int index)
{
    bool res = false;
    lock();
...
int LogModle::do_log(LogLevel nlevel, const tstring strbuffer)
{
    // get time string
...
int LogModle::log(LogLevel nlevel, char* sformat, ...)
{
    va_list args;
...
tostringstream& LogModle::operator<<(const LogLevel ll)
{
    lock();
    m_tmp = ll;
    return m_buff;
...
LogModle& operator<<(tostream& os, LogModle& loger)
{
    loger.flush();
    loger.unlock();
    return loger;
