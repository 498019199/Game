...
#define LOGER_ERROR(msg) \
    (g_Loger) << LogLevel::LOG_ERROR << ##msg## << (g_Loger)
#define LOGER_WARN(msg)\
    (g_Loger) << LogLevel::LOG_WARING << ##msg## << (g_Loger)
#define LOGER_INFO(msg)\
    (g_Loger) << LogLevel::LOG_INFO << ##msg## << (g_Loger)
#define LOGER_DEBUG(msg)\
    (g_Loger) << LogLevel::LOG_DEBUG << ##msg## << (g_Loger)
#define LOGER_TRACE(msg)\
    (g_Loger) << LogLevel::LOG_TRACE << ##msg## << (g_Loger)
//c style
...
public:
	...
	virtual void OnExcute(float fTime);
    ...
    friend LogModle& operator<<(tostream &os, LogModle& loger);
private:
    void unlock();
