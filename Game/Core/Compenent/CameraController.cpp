...
#include "../Container/C++17/iterator.h"
...
FirstPersonCameraController::FirstPersonCameraController(Context* pContext, bool use_input_engine)
	:CameraController(pContext), left_button_down_(false)
{
	if (use_input_engine)
	{
...
void FirstPersonCameraController::InputHandler(InputEngine const & ie, InputAction const & action)
{
	...
	if (camera_)
	{
		...
		case Turn:
			...
			default:
				UNREACHABLE_MSG("Invalid input type");
			}
			break;
...
void FirstPersonCameraController::AttachCamera(ICamera& camera)
{
	float3 scale, trans;
	Quaternion quat;
...
void TrackballCameraController::AttachCamera(ICamera& camera)
{
	CameraController::AttachCamera(camera);
...
//}
//
//void CameraPathController::AttachCamera(ICamera& camera)
//{
//	CameraController::AttachCamera(camera);
//
...
//{
//	camera_->BindUpdateFunc(std::function<void(ICamera&, float, float)>());
//
//	CameraController::DetachCamera();
//}
//
//void CameraPathController::UpdateCameraFunc(ICamera& /*camera*/, float app_time, float /*elapsed_time*/)
//{
