// 2018/8/24 比我写的好，我赖的重写来，直接移植
#ifndef _STRING_H_
#define _STRING_H_
#include "../Container/char_traits.h"
...
public:
	...
	{
		Resize(length);
		TCharTrains<char>::Copy(buffer_, str, length);
	}
	...
	String& operator =(const String& rhs)
	{
		...
		{
			Resize(rhs.length_);
			TCharTrains<char>::Copy(buffer_, rhs.buffer_, rhs.length_);
		}
	...
	String& operator =(const char* rhs)
	{
		...
		Resize(rhsLength);
		TCharTrains<char>::Copy(buffer_, rhs, rhsLength);
		return *this;
	}
	...
	String& operator +=(const String& rhs)
	{
		...
		TCharTrains<char>::Copy(buffer_ + oldLength, rhs.buffer_, rhs.length_);
		return *this;
	}
	...
	String& operator +=(const char* rhs)
	{
		...
		TCharTrains<char>::Copy(buffer_ + oldLength, rhs, rhsLength);
		return *this;
	}
	...
	String operator +(const String& rhs) const
	{
		...
		TCharTrains<char>::Copy(ret.buffer_, buffer_, length_);
		TCharTrains<char>::Copy(ret.buffer_ + length_, rhs.buffer_, rhs.length_);
		return ret;
	}
	...
	String operator +(const char* rhs) const
	{
		...
		TCharTrains<char>::Copy(ret.buffer_, buffer_, length_);
		TCharTrains<char>::Copy(ret.buffer_ + length_, rhs, rhsLength);
...
public:
	...
	static unsigned CWStringLength(const wchar_t* str) { return str ? (unsigned)wcslen(str) : 0; }
	...
	WString& operator =(const WString& rhs)
	{
		...
		{
			Resize(rhs.length_);
			TCharTrains<wchar_t>::Copy(buffer_, rhs.buffer_, rhs.length_);
		}
	...
	{
		unsigned rhsLength = CWStringLength(rhs);
		Resize(rhsLength);
		TCharTrains<wchar_t>::Copy(buffer_, rhs, rhsLength);
		return *this;
	}
private:
	/// String length.
