			...
			out float4 rt0, out float4 rt1, out float4 rt2)
{
	...
	rt0 = float4(float3(floor(enc255), residual.x * 16 + residual.y) / 255, metalness_glossiness.y);

	rt1 = float4(albedo, metalness_glossiness.x);

	rt2 = float4(motion_vec * 0.5f + 0.5f, occlusion, 1);
}
...
float2 GetMotionVec(float4 rt2)
{
	return rt2.xy * 2 - 1;
}

float GetOcclusion(float4 rt2)
{
	return rt2.z;
}
